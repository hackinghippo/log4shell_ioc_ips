...
echo "grep all IPs...."
   #filter for valid IPs in repo dir
	grep -hiRaE --exclude="README.md" --exclude="ips.txt" --exclude="script.sh" --exclude="stats.txt" \
	  -o  "^(1?[0-9][0-9]?|2([0-4][0-9]|5[0-5]))(\.(1?[0-9][0-9]?|2([0-4][0-9]|5[0-5]))){3}$" | \
	# Filter out RFC1918 IPs
	grep -Pv "(^192\.168(\.([0-9]|1?[0-9][0-9]|2([0-4][0-9]|5[0-5]))){2}$)|(^172\.(1[6-9]|2[0-9]|3[0-1])(\.(1?[0-9][0-9]?|2([0-4][0-9]|5[0-5]))){2}$)|(^10(\.(1?[0-9][0-9]?|2([0-4][0-9]|5[0-5]))){3}$)" | \
	#Filter out any entrie starting with 0 or 255 the first octet
	grep -Pv "^(0+|255)\." | \
	sort -u > $DIR/ips.txt
